...
SocketNamespace.prototype.packet = function (packet) {
  ...
  store.clients(this.flags.endpoint, function (clients) {
    clients.forEach(function (id) {
      ...
      if (volatile) { store.publish('volatile:' + id, packet); }
      else { store.client(id).publish(packet); }
    });
  });
...
SocketNamespace.prototype.emit = function (name) {
  if (name == 'connection' || name == 'newListener') { return EventEmitter.prototype.emit.apply(this, arguments); }
...
SocketNamespace.prototype.socket = function (sid, readable) {
  if (!this.sockets[sid]) { this.sockets[sid] = new Socket(this.manager, sid, this, readable); }
...
SocketNamespace.prototype.handlePacket = function (sessid, packet) {
  ...
  switch (packet.type) {
    ...
    case 'ack':
      if (socket.acks[packet.ackId]) { socket.acks[packet.ackId].apply(socket, packet.args); }
      else { this.log.info('unknown ack packet'); }
      break;
