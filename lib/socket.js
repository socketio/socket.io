...
Socket.prototype.emit = function(ev){
  if (~exports.events.indexOf(ev)) {
    ...
    if ('function' == typeof args[args.length - 1]) {
      ...
      debug('emitting packet with ack id %d for socket %s', this.nsp.ids, this.id);
...
Socket.prototype.join = function(room, fn){
  debug('joining room %s at %s', room, this.nsp.name);
  var self = this;
  if (~this.rooms.indexOf(room)) return this;
  this.adapter.add(this.id, room, function(err){
    if (err) return fn && fn(err);
    debug('joined room %s at %s', room, self.nsp.name);
...
Socket.prototype.leave = function(room, fn){
  debug('leave room %s at %s', room, this.nsp.name);
  var self = this;
  this.adapter.del(this.id, room, function(err){
    if (err) return fn && fn(err);
    debug('left room %s at %s', room, self.nsp.name);
...
Socket.prototype.ondisconnect = function(){
  debug('got disconnect packet for %s', this.id);
...
Socket.prototype.onclose = function(reason){
  ...
  debug('closing socket %s - reason %s', this.id, reason);
  this.leaveAll();
