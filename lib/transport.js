...
Transport.prototype.onForcedDisconnect = function () {
  if (!this.disconnected) {
    ...
    if (this.open) { this.packet({ type: 'disconnect' }); }
    this.end(true);
  }
};
...
Transport.prototype.end = function (forced, reason) {
  if (!this.disconnected) {
    ...
    if (!forced) { this.store.disconnect(this.id, false, reason); }
...
Transport.prototype.writeVolatile = function (msg) {
  if (this.open) {
    if (this.drained) { this.write(msg); }
    else { this.log.debug('ignoring volatile packet, buffer not drained'); }
  } else {
