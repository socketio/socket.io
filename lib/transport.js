...
Transport.prototype.handleRequest = function handleRequest (req) {
  ...
  if (req.method == 'GET') {
    ...
    this.store.publish('open:' + this.id, function publish () {
      self.store.once('open:' + self.id, function open () {
    ...
    this.store.once('disconnect-force:' + this.id, function forcedDisconnect () {
      self.onForcedDisconnect();
    });

    this.store.on('heartbeat-clear:' + this.id, function clearHeartbeat () {
...
/**
 * Calle when the connection has an error.
 *
 * @api private
 */
...
Transport.prototype.onForcedDisconnect = function onForcedDisconnect () {
...
Transport.prototype.clearHeartbeatTimeout = function clearHeartbeatTimeout () {
  if (this.heartbeatTimeout) {
...
Transport.prototype.disconnect = function disconnect (reason) {
  this.packet({ type: 'disconnect' });
...
Transport.prototype.end = function end (forced, reason) {
