...
function Manager (server) {
  ...
  /***if (this.get('transports').indexOf('flashsocket')) {
    var fpsPorts = [server];
  if (this.get('flash policy server')) {
    fpsPorts.push.call(fpsPorts, 843, self.get('flash policy server port'));
  }
  var fps = require('policyfile').createServer({
    log: self.log.info.bind(self.log)
  }).on('connect_failed', function() {
console.error('FAILED', this, arguments);
    });
    fps.listen(fpsPort, function() {
console.error('LIST', this, arguments);
    });
  }***/
...
Manager.prototype.set = function (key, value) {
  ...
  this.emit('update:' + key, this.settings[key], key);
  return this;
};
