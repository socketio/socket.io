...
function Manager (server, options) {
  ...
  this.settings = {
    ...
    //, 'flash policy server port': 10043
  ...
  if (this.get('transports').indexOf('flashsocket')) {
    transports.flashsocket.startPolicyServer(this);
  }
...
Manager.prototype.handleRequest = function (req, res) {
  ...
  if (!data) {
    this.log.debug('ignoring request outside socket.io namespace', req.method, req.url);
