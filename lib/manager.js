...
/**
 * Add an http server to listen to events of. enabels http and ssl on same socket.io
 *
 * @param {HTTPServer} server
 * @api public
 */
 
Manager.prototype.addServer = function (aserver) {
  this.log.debug('socket.io start addServer');
  var serverid=this.server.length;
  this.server.push(aserver);
  
  var self = this;

  // default error handler
  aserver.on('error', function(err) {
    self.log.warn('error raised: ' + err);
  });

  // reset listeners
  this.oldListeners.push(aserver.listeners('request').splice(0));
  
  aserver.on('request', function (req, res, next) {
    self.handleRequest(req, res, next, serverid, this);
  });

  aserver.on('upgrade', function (req, socket, head) {
  ...
  aserver.once('listening', function () {
    self.gc = setInterval(self.garbageCollection.bind(self), 10000);
  });
  
  if(serverid==0)
    this.log.debug('socket.io server added');  
  else
    this.log.info('socket.io server added'); 
    
  this.emit('addserver', aserver, this.server.length-1);
  return this;
};
...
Manager.prototype.handleRequest = function (req, res, next, serverid, myserver) {
  ...
  if (!data) {
    ...
    {
      var this_oldListeners=this.oldListeners[serverid];
      var this_server=this.servers[serverid];
      for (var i = 0, l = this_oldListeners.length; i < l; i++) {
        this_oldListeners[i].call(this_server, req, res);
      }
    }
    return;
  }
