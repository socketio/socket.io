...
Manager.prototype.configure = function (env, fn) {
  if ('function' == typeof env) { env.call(this); }
  else if (env == process.env.NODE_ENV) { fn.call(this); }
...
Manager.prototype.handleRequest = function (req, res) {
  ...
  if (data.protocol != protocol) {
    ...
    if (data.id) { this.handleHTTPRequest(data, req, res); }
    else { this.handleHandshake(data, req, res); }
  }
};
    ...
    self.store.client(data.id).count(function (err, count) {
      ...
      if (count == 1) {
        ...
        for (var i in self.namespaces) {
          ...
          if (i === '') { self.namespaces[i].handlePacket(data.id, { type: 'connect' }); }
        }
...
Manager.prototype.handleClientRequest = function (req, res, data) {
  ...
  function serve () {
    ...
    if (self.enabled('browser client etag') && cache.Etag) { headers.Etag = cache.Etag; }
...
Manager.prototype.transports = function (data) {
  ...
  for (var i = 0, l = transp.length; i < l; i++) {
    ...
    if (transport) {
      if (!transport.checkClient || transport.checkClient(data)) { ret.push(transport); }
    }
  }
...
Manager.prototype.of = function (nsp) {
  if (this.namespaces[nsp]) { return this.namespaces[nsp]; }
