...
Manager.prototype.checkRequest = function (req) {
  ...
  var match,
    reqUrl = req.url.substr(0, 4) === 'http' ? req.url.substr((req.connection.encrypted ? 8 : 7) + req.headers.host.length, req.url.length) : req.url;

  if (typeof resource === 'string') {
    match = reqUrl.substr(0, resource.length);
    if (match !== resource) match = null;
  } else {
    match = resource.exec(reqUrl);
    if (match) match = match[0];
  }

  if (match) {
    var uri = url.parse(reqUrl.substr(match.length), true)
      , path = uri.pathname || ''
