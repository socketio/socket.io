...
Manager.prototype.initStore = function () {
  ...
  this.store.subscribe('handshake', function (id) {
    self.onHandshake(id);
  });
...
Manager.prototype.onHandshake = function (id) {
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  this.authorize(data, function (err, authorized) {
    ...
    if (authorized) {
      ...
      self.onHandshake(id);
      self.store.publish('handshake', id);

      // remove unwanted data
      delete data.request;

      self.store.client(id).set('handshake', data);
