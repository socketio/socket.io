...
function Manager (server) {
  ...
  // flash policy inline server
  if (this.settings.flashPolicyServer) {
    var defaultPolicy =
    ...
    this.log.info('flash policy inline server started');
    server.on('connection', function(stream) {
      stream.once('data', function (data) {
        // Only check the initial data
        if (data[0] === 60 && data.length === 23) {
          if (data === '<policy-file-request/>\0') {
            self.log.info('Answering flash policy request inline');
