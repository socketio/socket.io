...
Manager.prototype.__defineGetter__('store', function store () {
...
Manager.prototype.__defineGetter__('log', function log () {
...
Manager.prototype.get = function get (key) {
...
Manager.prototype.set = function set (key, value) {
  if (arguments.length == 1) return this.get(key);
  
  this.settings[key] = value;
  EventEmitter.prototype.emit.call(this, 'set:' + key, this.settings[key], key);
  return this;
};
...
Manager.prototype.enable = function enable (key) {
...
Manager.prototype.disable = function disable (key) {
...
Manager.prototype.enabled = function enabled (key) {
  return !!this.settings[key];
};
...
Manager.prototype.disabled = function disabled (key) {
  return !this.settings[key];
};
...
Manager.prototype.configure = function configure (env, fn) {
...
Manager.prototype.handleRequest = function handleRequest (req, res) {
...
Manager.prototype.handleUpgrade = function handleUpgrade (req, socket, head) {
  var data = this.checkRequest(req)
    , self = this;
...
Manager.prototype.handleHTTPRequest = function handleHTTPRequest (data, req, res) {
  req.res = res;
  this.handleClient(data, req);
};
...
Manager.prototype.handleClient = function handleClient (data, req) {
  ...
  if (req.socket.__ioTransport) {
    // if the request comes through a validated socket, avoid roundtrips
    transport.resume();
    return;
  }

  this.store.isHandshaken(data.id, function handshaken (err, handshaken) {
    if (err || !handshaken) {
    ...
    self.store.client(data.id).count(function count (err, count) {
      if (count == 1) {
        ...
        self.store.on('message:' + data.id, function message (packet) {
          self.log.info('manager received client packet');
          self.handlePacket(data.id, packet);
        });
      }

      req.socket.__ioTransport = transport;
      transport.resume();
    });
  });
};
...
Manager.prototype.handleClientRequest = function handleClientRequest (req, res) {
  var self = this;
  ...
  if (!this.client) {
    if (this.get('browser client')) {
      ...
      fs.readFile(__dirname + '/client/socket.io.min.js', function readFile (err, data) {
        if (err) {
          res.writeHead(500);
...
Manager.prototype.handleHandshake = function handleHandshake (data, req, res) {
  var self = this;
  ...
  this.authorize(data, function authorize (err, authorized) {
    if (err) return error(err);

    self.log.info('handshake ' + (authorized ? 'authorized' : 'unauthorized'));

    if (authorized) {
      self.store.handshake(data, function handshake (err, id) {
        if (err) return error(err);
...
Manager.prototype.verifyOrigin = function verifyOrigin (request) {
  ...
  if (origin) {
    try {
      ...
      return ~origins.indexOf(parts.host + ':' + parts.port) ||
...
Manager.prototype.handlePacket = function handlePacket (sessid, packet) {
...
Manager.prototype.authorize = function authorize (data, fn) {
  if (this.get('authorization')) {
    var self = this;

    this.get('authorization').call(this, data, function getAuthorization (err, authorized) {
...
Manager.prototype.transports = function transports (data) {
...
Manager.prototype.checkRequest = function checkRequest (req) {
...
Manager.prototype.for = function forNamespace(nsp) {
