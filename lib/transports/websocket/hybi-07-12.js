...
function WebSocket (mng, data, req) {
  ...
  this.manager = mng;
  this.parser = new Parser(this.manager.get('destroy buffer size'));
...
function Parser (max) {
  ...
  this.currentMessage = '';
  this.maximumMessageSize = max || 10E7;
  ...
  this.opcodeHandlers = {
    ...
    '1': function(data) {
      ...
      var expectData = function(length) {
        if (self.currentMessage.length + length > self.maximumMessageSize) {
          self.error('message too large');
          return;
        }
    ...
    '2': function(data) {
      ...
      var sumLength = function(prev, buf) {
        return prev + buf.length;
      }

      var expectData = function(length) {
        if (self.currentMessage.reduce(sumLength, length) > self.maximumMessageSize) {
          self.error('message too large');
          return;
        }
        if (self.state.masked) {
