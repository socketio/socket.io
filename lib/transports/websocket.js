...
WebSocket.prototype.onSocketConnect = function () {
  ...
  if (waitingForNonce) { this.socket.setEncoding('binary'); }
  else if (this.proveReception(headers)) { self.flush(); }
  ...
  this.socket.on('data', function (data) {
    if (waitingForNonce) {
      ...
      if (self.proveReception(headers)) { self.flush(); }
...
WebSocket.prototype.write = function (data) {
  if (this.open) {
    ...
    try {
      if (this.socket.write(buffer)) { this.drained = true; }
    } catch (e) {
      this.end();
    }
...
Parser.prototype.parse = function () {
  for (var i = this.i, chr, l = this.buffer.length; i < l; i++){
    ...
    if (i === 0) {
      if (chr != '\u0000') { this.error('Bad framing. Expected null byte as first frame'); }
      else { continue; }
    }
