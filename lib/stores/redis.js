...
Client.prototype.get = function (key, fn) {
  if (arguments.length == 2) this.store.cmd.hget(this.id, key, fn);
  else {
    var cmd = this.store.cmd
      , args = Array.prototype.slice.call(arguments)
      , fn = args.pop()
      , callback = function(err, res) {
          if (err) return fn(err);
          var data = args.reduce(function(acc, arg, i) {
            acc[arg] = (res[i] === undefined) ? null : res[i];
            return  acc;
          }, {});
          fn(null, data);
        };
    cmd.hmget.apply(cmd, [this.id].concat(args, callback));
  }
  return this;
};
