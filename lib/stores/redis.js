...
function Redis (opts) {
  ...
  self.sub.on('message', function message (ch, msg) {
    var list = self.messageListeners[ch];
    msg = self.unpack(msg);
...
Redis.prototype.subscribe = function (name, consumer, fn) {
  ...
  if (consumer || fn) {
    ...
    self.sub.on('subscribe', function subscribe (ch) {
      if (name == ch) {
        if (self.messageListeners[ch] == null) {
          self.messageListeners[ch] = [];
        }
        self.messageListeners[ch].push(consumer);

        self.on('unsubscribe', function unsubscribe (ch) {
          if (name == ch) {
            var list = self.messageListeners[ch];
            list.splice(list.indexOf(consumer), 1);
