...
Static.prototype.init = function () {
  ...
  function id (transports) {
    return transports.join('').split('').map(function (char) {
  ...
  this.add('/socket.io+', {}, function (path, callback) {
    var available = self.manager.get('transports')
      , matches = /+((?:\+)?[\w\-]+)*(?:\.js)$/g.exec(path)
      , transports = [];
    ...
    if (!transports.lengt) return callback('No valid transports');
