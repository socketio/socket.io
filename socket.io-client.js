...
if ('undefined' != typeof process) {
  ...
  IO.source = read('./socket.io-client.js');
}
...
IO.connect = function(uri){
  if (!IO.global) IO.global = IO(uri).io;
  var nsp = url(uri).pathname;
  return IO.global.create(nsp);
};
...
require.register("socket.io/lib/io.js", function(module, exports, require){
 ...
 * `IO` manager.
 *
 * @param {String} optional, base uri
 ...
 * Opens the given `uri`.
 *
 * @param {String|Object} url or object
 * @return {Socket} for the url `path`, defaulting to `/`
 * @api public
 */

IO.prototype.open = function(obj){
  if ('string' == typeof obj) obj = this.parse(obj);
  this.engine = new eio.Socket(obj);
  this.engine.on('close', bind(this, 'onclose'));
  return this.create(obj.nsp);
};
...
IO.prototype.create = function(nsp){
...
IO.prototype.destroy = function(socket){
...
Socket.prototype.connect = function(){
  ...
  if ('open' == socket.readyState) this.onopen();
  this.subs = [
    socket.on('open', bind(this, 'onopen')),
    socket.on('error', bind(this, 'onerror'))
  ];
};
...
Socket.prototype.onopen = function(){
  this.engine.write();
  this.subs.push(
    this.io.on('packet:' + this.nsp, bind(this, 'onpacket')),
...
Socket.prototype.onpacket = function(packet){
  switch (packet.type) {
    case parser.PACKET_CONNECT:
      break;

    case parser.PACKET_EVENT:
      break;

    case parser.PACKET_ACK:
      break;

    case parser.PACKET_DISCONNECT:
      break;
  }
};
