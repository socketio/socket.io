...
const s = {
    ...
    r = ({
        ...
        return n && s instanceof Blob ? r ? h(s) : o(s, h) : i && (s instanceof ArrayBuffer || (a = s, "function" == typeof ArrayBuffer.isView ? ArrayBuffer.isView(a) : a && a.buffer instanceof ArrayBuffer)) ? r ? h(s) : o(new Blob([s]), h) : h(t[e] + (s || ""));
        var a
    },
    o = (t, e) => {
...
const c = "function" == typeof ArrayBuffer,
    ...
    u = (t, e) => {
        if (c) {
            const s = (t => {
                let e, s, n, i, r, o = .75 * t.length,
                    h = t.length,
                    c = 0;
                "=" === t[t.length - 1] && (o--, "=" === t[t.length - 2] && o--);
                const p = new ArrayBuffer(o),
                    u = new Uint8Array(p);
                for (e = 0; e < h; e += 4) s = a[t.charCodeAt(e)], n = a[t.charCodeAt(e + 1)], i = a[t.charCodeAt(e + 2)], r = a[t.charCodeAt(e + 3)], u[c++] = s << 2 | n >> 4, u[c++] = (15 & n) << 4 | i >> 2, u[c++] = (3 & i) << 6 | 63 & r;
                return p
            })(t);
            return l(s, e)
        }
...
f.prototype.on = f.prototype.addEventListener = function (t, e) {
    ...
    return this
}, f.prototype.emitReserved = f.prototype.emit, f.prototype.listeners = function (t) {
...
function v(t, e) {
    e.useNativeTimers ? (t.setTimeoutFn = m.bind(y), t.clearTimeoutFn = b.bind(y)) : (t.setTimeoutFn = setTimeout.bind(y), t.clearTimeoutFn = clearTimeout.bind(y))
}
...
class w extends f {
    constructor(t) {
        super(), this.writable = !1, v(this, t), this.opts = t, this.query = t.query, this.readyState = "", this.socket = t.socket
    }
    onError(t, e, s) {
...
const _ = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_".split(""),
    E = {};
let A, R = 0,
    O = 0;
...
class j extends f {
    constructor(t, e) {
        super(), v(this, e), this.opts = e, this.method = e.method || "GET", this.uri = t, this.async = !1 !== e.async, this.data = void 0 !== e.data ? e.data : null, this.create()
    }
    create() {
        const t = g(this.opts, "agent", "pfx", "key", "passphrase", "cert", "ca", "ciphers", "rejectUnauthorized", "autoUnref");
        t.xdomain = !!this.opts.xd, t.xscheme = !!this.opts.xs;
        const e = this.xhr = new S(t);
        try {
        ...
        "undefined" != typeof document && (this.index = j.requestsCount++, j.requests[this.index] = this)
    }
    onError(t) {
    ...
    cleanup(t) {
        if (void 0 !== this.xhr && null !== this.xhr) {
            if (this.xhr.onreadystatechange = L, t) try {
                this.xhr.abort()
            } catch (t) {}
            "undefined" != typeof document && delete j.requests[this.index], this.xhr = null
        }
    }
    onLoad() {
...
if (j.requestsCount = 0, j.requests = {}, "undefined" != typeof document)
    if ("function" == typeof attachEvent) attachEvent("onunload", q);
    else if ("function" == typeof addEventListener) {
    addEventListener("onpagehide" in y ? "pagehide" : "unload", q, !1)
}

function q() {
    for (let t in j.requests) j.requests.hasOwnProperty(t) && j.requests[t].abort()
}
const I = "function" == typeof Promise && "function" == typeof Promise.resolve ? t => Promise.resolve().then(t) : (t, e) => e(t, 0),
...
const M = {
        websocket: class extends w {
            ...
            doOpen() {
                ...
                const t = this.uri(),
                    ...
                    s = F ? {} : g(this.opts, "agent", "perMessageDeflate", "pfx", "key", "passphrase", "cert", "ca", "ciphers", "rejectUnauthorized", "localAddress", "protocolVersion", "origin", "maxPayload", "family", "checkServerIdentity");
            ...
            uri() {
                ...
                let s = "";
                this.opts.port && ("wss" === e && 443 !== Number(this.opts.port) || "ws" === e && 80 !== Number(this.opts.port)) && (s = ":" + this.opts.port), this.opts.timestampRequests && (t[this.opts.timestampParam] = T()), this.supportsBinary || (t.b64 = 1);
                const n = B(t);
        ...
        polling: class extends w {
            ...
            uri() {
                ...
                let s = "";
                !1 !== this.opts.timestampRequests && (t[this.opts.timestampParam] = T()), this.supportsBinary || t.sid || (t.b64 = 1), this.opts.port && ("https" === e && 443 !== Number(this.opts.port) || "http" === e && 80 !== Number(this.opts.port)) && (s = ":" + this.opts.port);
                const n = B(t);
                return e + "://" + (-1 !== this.opts.hostname.indexOf(":") ? "[" + this.opts.hostname + "]" : this.opts.hostname) + s + this.opts.path + (n.length ? "?" + n : "")
            }
            request(t = {}) {
...
class K extends f {
    ...
    open() {
        let t;
        if (this.opts.rememberUpgrade && K.priorWebsocketSuccess && -1 !== this.transports.indexOf("websocket")) t = "websocket";
        else {
    ...
    probe(t) {
            ...
            s || (e.send([{
                ...
                if (!s)
                    if ("pong" === t.type && "probe" === t.data) {
                        ...
                        K.priorWebsocketSuccess = "websocket" === e.name, this.transport.pause((() => {
        ...
        const c = () => {
            e.removeListener("open", n), e.removeListener("error", r), e.removeListener("close", o), this.off("close", h), this.off("upgrading", a)
        };
        e.once("open", n), e.once("error", r), e.once("close", o), this.once("close", h), this.once("upgrading", a), e.open()
    }
    onOpen() {
        if (this.readyState = "open", K.priorWebsocketSuccess = "websocket" === this.transport.name, this.emitReserved("open"), this.flush(), "open" === this.readyState && this.opts.upgrade && this.transport.pause) {
            let t = 0;
...
const Y = "function" == typeof ArrayBuffer,
    ...
    W = "function" == typeof Blob || "undefined" != typeof Blob && "[object BlobConstructor]" === z.call(Blob),
    $ = "function" == typeof File || "undefined" != typeof File && "[object FileConstructor]" === z.call(File);

function J(t) {
    return Y && (t instanceof ArrayBuffer || (t => "function" == typeof ArrayBuffer.isView ? ArrayBuffer.isView(t) : t.buffer instanceof ArrayBuffer)(t)) || W && t instanceof Blob || $ && t instanceof File
}
...
function X(t, e) {
    ...
    if (J(t)) return !0;
    if (t.toJSON && "function" == typeof t.toJSON && 1 === arguments.length) return X(t.toJSON(), !0);
    for (const e in t)
        if (Object.prototype.hasOwnProperty.call(t, e) && X(t[e])) return !0;
    return !1
}
...
function G(t) {
    const e = [],
        s = t.data,
        n = t;
    return n.data = Q(s, e), n.attachments = e.length, {
        packet: n,
        buffers: e
    }
}
...
function Q(t, e) {
    ...
    if (Array.isArray(t)) {
        ...
        for (let n = 0; n < t.length; n++) s[n] = Q(t[n], e);
        return s
    }
    if ("object" == typeof t && !(t instanceof Date)) {
        const s = {};
        for (const n in t) Object.prototype.hasOwnProperty.call(t, n) && (s[n] = Q(t[n], e));
        return s
    }
    return t
}
...
function Z(t, e) {
    return t.data = tt(t.data, e), t.attachments = void 0, t
}
...
function tt(t, e) {
    ...
    if (Array.isArray(t))
        for (let s = 0; s < t.length; s++) t[s] = tt(t[s], e);
    else if ("object" == typeof t)
        for (const s in t) Object.prototype.hasOwnProperty.call(t, s) && (t[s] = tt(t[s], e));
    return t
}
const et = 5;
var st;
...
class nt extends f {
    ...
    add(t) {
        let e;
        if ("string" == typeof t) e = this.decodeString(t), e.type === st.BINARY_EVENT || e.type === st.BINARY_ACK ? (this.reconstructor = new it(e), 0 === e.attachments && super.emitReserved("decoded", e)) : super.emitReserved("decoded", e);
        else {
    ...
    decodeString(t) {
        ...
        if (t.charAt(++e)) {
            ...
            if (!nt.isPayloadValid(s.type, n)) throw new Error("invalid payload");
            s.data = n
        }
        return s
    }
...
var rt = Object.freeze({
    ...
    Encoder: class {
        ...
        encode(t) {
            return t.type !== st.EVENT && t.type !== st.ACK || !X(t) ? [this.encodeAsString(t)] : (t.type = t.type === st.EVENT ? st.BINARY_EVENT : st.BINARY_ACK, this.encodeAsBinary(t))
        }
        encodeAsString(t) {
            let e = "" + t.type;
            return t.type !== st.BINARY_EVENT && t.type !== st.BINARY_ACK || (e += t.attachments + "-"), t.nsp && "/" !== t.nsp && (e += t.nsp + ","), null != t.id && (e += t.id), null != t.data && (e += JSON.stringify(t.data, this.replacer)), e
        }
        encodeAsBinary(t) {
            const e = G(t),
...
class at extends f {
    ...
    emit(t, ...e) {
        if (ht.hasOwnProperty(t)) throw new Error('"' + t.toString() + '" is a reserved event name');
        e.unshift(t);
        const s = {
            type: st.EVENT,
            data: e,
...
class pt extends f {
    constructor(t, e) {
        var s;
        super(), this.nsps = {}, this.subs = [], t && "object" == typeof t && (e = t, t = void 0), (e = e || {}).path = e.path || "/socket.io", this.opts = e, v(this, e), this.reconnection(!1 !== e.reconnection), this.reconnectionAttempts(e.reconnectionAttempts || 1 / 0), this.reconnectionDelay(e.reconnectionDelay || 1e3), this.reconnectionDelayMax(e.reconnectionDelayMax || 5e3), this.randomizationFactor(null !== (s = e.randomizationFactor) && void 0 !== s ? s : .5), this.backoff = new ct({
    ...
    socket(t, e) {
        let s = this.nsps[t];
        return s || (s = new at(this, t, e), this.nsps[t] = s), s
    }
    _destroy(t) {
...
function lt(t, e) {
    ...
    const s = function (t, e = "", s) {
            let n = t;
            s = s || "undefined" != typeof location && location, null == t && (t = s.protocol + "//" + s.host), "string" == typeof t && ("/" === t.charAt(0) && (t = "/" === t.charAt(1) ? s.protocol + t : s.host + t), /^(https?|wss?):\/\//.test(t) || (t = void 0 !== s ? s.protocol + "//" + t : "https://" + t), n = H(t)), n.port || (/^(http|ws)$/.test(n.protocol) ? n.port = "80" : /^(http|ws)s$/.test(n.protocol) && (n.port = "443")), n.path = n.path || "/";
        ...
        i = s.id,
        r = s.path,
        o = ut[i] && r in ut[i].nsps;
    let h;
    return e.forceNew || e["force new connection"] || !1 === e.multiplex || o ? h = new pt(n, e) : (ut[i] || (ut[i] = new pt(n, e)), h = ut[i]), s.query && !e.query && (e.query = s.queryKey), h.socket(s.path, e)
}
Object.assign(lt, {
    Manager: pt,
    Socket: at,
...
export {
    pt as Manager, at as Socket, lt as connect, lt as
    default, lt as io, et as protocol
};
